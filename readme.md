...
```
**注意：如果你的`admin_users`表已经存在`mfa_enabled`, `mfa_secret`, `last_mfa_login`, `last_backup_login`字段，请跳过SQL中`admin_users`的`ALTER TABLE`部分，并在更新代码部署后手动到D1控制台执行`DELETE FROM mfa_secrets;`，因为新代码已将mfa secret字段移回admin_users表。**
...
*   在此页面，您可以：
    *   **系统概览**：查看系统状态和访问摘要
    *   **域名管理**：添加/编辑/删除优选域名
    *   **IP 资源池管理**：**管理 IP 池，包括新的 HostMonit IPv6 开启按钮，并显示 IP 来源**，以及自动更新设置
...
| 用户浏览器  | `mg_worker.js`     | `/api/ips/refresh`                  | 手动更新IP           |
...
| `POST`   | `/api/ips/refresh`          | 手动更新IP池               | `{"hostmonit_v4":true, "hostmonit_v6":false, "vps789":true}` |
...
2.  **IP 更新 [v2.1更新]**：
    ...
    *   **新增 HostMonit IPv6 接口，可在管理后台手动开启获取 IPv6 地址。**
